...
function App() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setUploadProgress({ progress: 10, status: 'uploading', message: 'Uploading file...' });
      ...
      xhr.upload.addEventListener('progress', (e) => {
        if (e.lengthComputable) {
          ...
          setUploadProgress({ progress, status: 'uploading', message: 'Uploading file...' });
        }
      });
      ...
      xhr.addEventListener('load', async () => {
        if (xhr.status === 200) {
          setUploadProgress({ progress: 50, status: 'processing', message: 'Processing document...' });
  ...
  const pollAnalysisStatus = async () => {
    ...
    const poll = async () => {
      try {
        attempts++;
        setUploadProgress({ 
          progress: 50 + (attempts * 2), 
          status: 'analyzing', 
          message: `Analyzing document... (${attempts}/${maxAttempts})` 
        });

        const [analysisRes, summaryRes, riskRes] = await Promise.all([
          fetch(`${API_BASE}/api/esg-analysis`, { 
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
          }),
          fetch(`${API_BASE}/api/esg-summary`, { 
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
          }),
          fetch(`${API_BASE}/api/esg-risk-level`, { 
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
          })
        ]);

        if (analysisRes.ok && summaryRes.ok && riskRes.ok) {
  ...
  const downloadReport = async () => {
    ...
    try {
      const [analysisRes, summaryRes, riskLevelRes] = await Promise.all([
        fetch(`${API_BASE}/api/esg-analysis`),
        fetch(`${API_BASE}/api/esg-summary`),
        fetch(`${API_BASE}/api/esg-risk-level`)
      ]);

      const esgAnalysis = await analysisRes.json() || [];
      const summaryData = await summaryRes.json();
      const riskData = await riskLevelRes.json();
      
      const esgSummary = summaryData.summary || "No summary available.";
      const esgRiskLevel = riskData.risk_level || "Unknown";

      if (!esgAnalysis.length) {
  ...
  const fetchEsgAnalysis = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/esg-analysis`);
      if (response.ok) {
        const data = await response.json();
        const analysisData = data.map((item: any) => ({
          category: item.Category || item.category,
          risk_percentage: item["Risk Percentage (%)"] || item.risk_percentage,
          score: item["Weighted ESG Risk Score"] || item.score,
        }));
        setEsgAnalysis(analysisData);
      }
    } catch (error) {
            ...
            <Route
              ...
              element={
                <>
                  ...
                  <div
                    className="h-64 w-full bg-cover bg-center object-cover lg:h-96 rounded-xl mb-8"
                    style={{
                  ...
                  <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-lg border p-8 mb-8 transition-colors duration-300`}>
                    <div className="text-center">
                      ...
                      <div className={`border-2 border-dashed ${isDarkMode ? 'hover:border-emerald-500 border-gray-600' : 'border-gray-300 hover:border-emerald-400'} rounded-lg p-8 transition-colors duration-200`}>
                        <input
                          type="file"
                          ...
                          className="hidden"
                          id="file-upload"
                          disabled={uploadProgress.status === 'uploading' || uploadProgress.status === 'processing' || uploadProgress.status === 'analyzing'}
                        />
                        <label
                          htmlFor="file-upload"
                          className={`cursor-pointer flex flex-col items-center ${uploadProgress.status === 'uploading' || uploadProgress.status === 'processing' || uploadProgress.status === 'analyzing' ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                      ...
                      {uploadedFile && uploadProgress.status === 'complete' && (
                  ...
                  {reportGenerated && (
                    <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-lg border p-6 sm:p-8 mb-8 transition-colors duration-300`}>
